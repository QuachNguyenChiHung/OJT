...
exports.handler = async (event) => {
  try {
    ...
    return successResponse({
      averageRating: parseFloat(row.avg_rating || 0).toFixed(1),
      avgRating: parseFloat(row.avg_rating || 0).toFixed(1), // alias for compatibility
    ...
    return errorResponse(error.message || 'Failed to get rating stats', 500);
  }
};

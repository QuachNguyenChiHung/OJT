...
public class DataInitializer {
    ...
    void seedTransactional(
        ...
        for (int i = 0; i < 10; i++) {
            ...
            p.setPrice(new BigDecimal((i + 1) * 10));
        ...
        for (int i = 0; i < 10; i++) {
            ...
            // use the first product detail for each product (index i*5)
            od.setProductDetails(pds.get(i * 5));
        ...
        int[] ratingValues = {5,4,3,2,1,5,4,3,2,5};
        for (int i = 0; i < 10; i++) {
            Rating r = new Rating();
            // DO NOT set ID manually
            r.setUser(users.get(i));
            r.setProduct(products.get(i));
            r.setRatingValue(ratingValues[i]);
            ratings.add(r);
        }
        // Persist ratings explicitly by saving via product (product has cascade on ratings)
